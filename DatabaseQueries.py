...
def insert_datatable_with_table_director(config_dict, db_config, sql_table_name, column_names_list, df_row):
    ...
    if sql_table_name == 'authorized_signatories':
        ...
        if len(result) == 0:  # If no matching record found
            ...
            db_cursor.execute(update_query)
            logging.info(f"Data row values are saved in table '{sql_table_name}' with \n {df_row}")
    elif sql_table_name == 'open_charges':
        ...
        if len(result) == 0:  # If no matching record found
            ...
            db_cursor.execute(update_query)
            logging.info(f"Data row values are saved in table '{sql_table_name}' with \n {df_row}")
    elif sql_table_name == 'current_shareholdings':
        shareholders_name_column_name = config_dict['name_column_name_in_db_shareholders']
        shareholders_name = result_dict[shareholders_name_column_name]
        currency_column_name = config_dict['currency_column_name']
        currency = result_dict[currency_column_name]
        select_query = (
            f'SELECT * FROM {sql_table_name} WHERE {registration_column_name} = "{registration_no}" AND {shareholders_name_column_name}'
            f' = "{shareholders_name}" and {currency_column_name} = "{currency}"')
        logging.info(select_query)
        db_cursor.execute(select_query)
        result = db_cursor.fetchall()
        logging.info(len(result))
        if len(result) == 0:  # If no matching record found
            ...
            result_dict.pop(registration_column_name)
            result_dict.pop(shareholders_name_column_name)
            result_dict.pop(currency_column_name)
            column_names_list = list(column_names_list)
            column_names_list.remove(registration_column_name)
            column_names_list.remove(shareholders_name_column_name)
            column_names_list.remove(currency_column_name)
            update_query = f'''UPDATE {sql_table_name}
                                                                       SET {', '.join([f'{col} = "{str(result_dict[col])}"' for col in column_names_list])} 
                                                                       WHERE {registration_column_name} = "{registration_no}" AND {shareholders_name_column_name} = "{shareholders_name}" and {currency_column_name} = "{currency}"'''
            logging.info(update_query)
            db_cursor.execute(update_query)
            logging.info(f"Data row values are saved in table '{sql_table_name}' with \n {df_row}")
    elif sql_table_name == 'principal_business_activities':
        primary_activity_code_column_name = config_dict['primary_activity_code_column_name']
        primary_activity_code = result_dict[primary_activity_code_column_name]
        secondary_activity_code_column_name = config_dict['secondary_activity_code_column_name']
        secondary_activity_code = result_dict[secondary_activity_code_column_name]
        select_query = (
            f'SELECT * FROM {sql_table_name} WHERE {registration_column_name} = "{registration_no}" AND {primary_activity_code_column_name}'
            f' = "{primary_activity_code}" and {secondary_activity_code_column_name} = "{secondary_activity_code}"')
        logging.info(select_query)
        db_cursor.execute(select_query)
        result = db_cursor.fetchall()
        logging.info(len(result))
        if len(result) == 0:  # If no matching record found
            ...
            result_dict.pop(registration_column_name)
            result_dict.pop(primary_activity_code_column_name)
            result_dict.pop(secondary_activity_code_column_name)
            column_names_list = list(column_names_list)
            column_names_list.remove(registration_column_name)
            column_names_list.remove(primary_activity_code_column_name)
            column_names_list.remove(secondary_activity_code_column_name)
            update_query = f'''UPDATE {sql_table_name}
                                                                               SET {', '.join([f'{col} = "{str(result_dict[col])}"' for col in column_names_list])} 
                                                                               WHERE {registration_column_name} = "{registration_no}" AND {primary_activity_code_column_name} = "{primary_activity_code}" and {secondary_activity_code_column_name} = "{secondary_activity_code}"'''
            logging.info(update_query)
            db_cursor.execute(update_query)
            logging.info(f"Data row values are saved in table '{sql_table_name}' with \n {df_row}")
