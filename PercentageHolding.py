...
def get_percentage_holding(db_config, registration_no):
    ...
    try:
        ...
        paid_up_capital_query = f"select total_equity_shares from shareholdings_summary where registration_no = '{registration_no}'"
        logging.info(paid_up_capital_query)
        cursor.execute(paid_up_capital_query)
        paid_up_capital = cursor.fetchone()[0]
        paid_up_capital = str(paid_up_capital).replace(',', '')
        try:
            total_paid_up_capital = float(paid_up_capital)
        except:
